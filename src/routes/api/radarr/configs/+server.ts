...
import {
  getUserRadarrConfigs,
  createRadarrConfig,
  updateRadarrConfig,
  deleteRadarrConfig,
  getRadarrConfig
} from '$lib/server/radarr';

// GET - List user's Radarr configs
export const GET: RequestHandler = async ({ locals }) => {
  if (!locals.user) {
    return json({ error: 'Unauthorized' }, { status: 401 });
  }

  const configs = getUserRadarrConfigs(locals.user.id);
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    const config = createRadarrConfig(userId, name, url, apiKey, isDefault);
    ...
    console.error('Failed to create Radarr config:', error);
    return json({ error: 'Failed to create configuration' }, { status: 500 });
  }
};

// PUT - Update Radarr config
export const PUT: RequestHandler = async ({ request, locals }) => {
  ...
  const config = updateRadarrConfig(id, locals.user.id, { name, url, apiKey, isDefault });
...
export const DELETE: RequestHandler = async ({ url, locals }) => {
  ...
  const deleted = deleteRadarrConfig(id, locals.user.id);
